 ...
 * @return 
 */
int serial_init_device(int index){
...
int serial_send_byte(int index, unsigned char c){
  volatile struct h8_3069f_sci* sci = regs[index].sci;
  while(!serial_is_send_enable(index)); /* wait for send*/
  sci->tdr = c;
  sci->ssr &= ~H8_3069F_SCI_SSR_TDRE; /*send*/
  return 0;
}
